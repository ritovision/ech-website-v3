...
const isCurrentPath = (href: string, path: string) => path === href;

const isPathInLineage = (href: string, path: string) =>
  path === href || path.startsWith(`${href}/`);

const isNavItemActive = (item: Nav, path: string): boolean => {
  if (item.link && !isExternalLink(item.link) && isPathInLineage(item.link, path)) {
    return true;
  }

  return item.children?.some((child) => isNavItemActive(child, path)) ?? false;
};
...
export default function Navbar() {
  ...
  const NavLink = ({
    ...
    const currentPage = !external && isCurrentPath(href, path);
    const classes = cn(
      "block font-antonio transition-colors duration-200",
      currentPage ? activeClassName : inactiveClassName,
      className
    );
    ...
    >
      ...
      <nav className="hidden xl:flex gap-x-16 items-center absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2">
        {nav.map((item) => {
          ...
          const sectionActive = isNavItemActive(item, path);
          if (hasChildren) {
            return (
              <DesktopNavPopover
                key={item.label}
                ...
                renderLink={(linkItem, className) => (
                  <NavLink
                    item={linkItem}
                    className={className}
                    activeClassName="text-white underline decoration-2 underline-offset-4"
          ...
          return (
            <NavLink
              ...
              item={item}
              className="text-3xl"
              activeClassName="text-black underline decoration-2 underline-offset-8"
                          ...
                          <div key={item.label}>
                            {item.children && item.children.length > 0 ? (
                              <button
                                type="button"
                                className={cn(
                                  "text-5xl font-antonio text-white decoration-2 underline-offset-8 transition-colors duration-200",
                          ...
                          if (item.children && item.children.length > 0) {
                            return (
                              ...
                              >
                                <AccordionTrigger
                                  className={cn(
                                    "text-3xl font-antonio text-white decoration-2 underline-offset-4",
                                    isNavItemActive(item, path)
                                      ? "underline"
                                      : ""
                                  )}
    ...
    <Popover>
      <PopoverTrigger asChild>
        <button
          type="button"
          className={cn(
            "text-3xl font-antonio decoration-2 underline-offset-8 transition-colors duration-200",
            isActive
              ...
              return (
                ...
                >
                  ...
                  >
                    <AccordionTrigger
                      className={cn(
                        "text-2xl font-antonio text-white decoration-2 underline-offset-4",
